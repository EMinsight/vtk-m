...
public:
  ...
  VTKM_CONT
  ArrayHandleVariantBase(const ArrayHandleVariantBase<TypeList>& src)
  ...
  VTKM_CONT
  vtkm::cont::ArrayHandleVariantBase<TypeList>& operator=(
  ...
  {
    return ArrayHandleVariantBase<TypeList>{ this->ArrayContainer->NewInstance() };
  }
...
struct ArrayHandleVariantTry
{
                  ...
                  Args&&... args) const
  {
    if (!called)
    {
      const auto* downcastContainer =
        dynamic_cast<const vtkm::cont::internal::ArrayHandleVariantContainer<T>*>(&container);
