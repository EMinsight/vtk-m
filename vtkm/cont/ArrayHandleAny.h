...
private:
  ...
  // when either the type_info is 'StorageAny<T,S>' or 'S'. That is why
  // we need to override the default  implementation.
  bool IsSameType(const std::type_info& other) const
  {
    return other == typeid(S) || other == typeid(*this);
  }
                 ...
                 Args&&... args)
{
  ...
  // clang-format off
extern template class VTKM_CONT_TEMPLATE_EXPORT StorageAny<vtkm::UInt8,       vtkm::cont::StorageTagBasic>;
extern template class VTKM_CONT_TEMPLATE_EXPORT StorageAny<vtkm::IdComponent, vtkm::cont::StorageTagBasic>;
extern template class VTKM_CONT_TEMPLATE_EXPORT StorageAny<vtkm::Id,          vtkm::cont::StorageTagBasic>;
  // clang-format on
#endif
