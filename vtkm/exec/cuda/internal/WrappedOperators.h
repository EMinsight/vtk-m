...
struct WrappedBinaryOperator
{
  ...
  VTKM_EXEC T operator()(const T* const x, const T& y) const
  {
    return m_f(*x, y);
  }

  VTKM_EXEC T operator()(const T& x, const T* const y) const
  {
    return m_f(x, *y);
  }

  VTKM_EXEC T operator()(const T* const x, const T* const y) const
  {
    return m_f(*x, *y);
  }
};
...
struct WrappedBinaryPredicate
{
  ...
  VTKM_EXEC bool operator()(const T* const x, const T& y) const
  {
    return m_f(*x, y);
  }

  VTKM_EXEC bool operator()(const T& x, const T* const y) const
  {
    return m_f(x, *y);
  }

  VTKM_EXEC bool operator()(const T* const x, const T* const y) const
  {
    return m_f(*x, *y);
  }
};
